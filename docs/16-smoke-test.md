...
```bash
kubectl create secret generic k8s-cluster01 \
  --from-literal="mykey=mydata"
```

Print a hexdump of the `k8s-cluster01` secret stored in etcd:
...
sudo ETCDCTL_API=3 etcdctl get \
  ...
  /registry/secrets/default/k8s-cluster01 | hexdump -C
```
...
Cleanup:
```bash
kubectl delete secret k8s-cluster01
```
