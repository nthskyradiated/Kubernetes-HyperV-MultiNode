...
Bootstrap the Kubernetes control plane across 2 compute instances and configure it for high availability. You will also create an external load balancer that exposes the Kubernetes API Servers to remote clients. The following components will be installed on each node: Kubernetes API Server, Scheduler, and Controller Manager.
...
*IMPORTANT: Since the ComponentStatus is being deprecated, the 2nd etcd instance might not be shown. Use the below command instead:
